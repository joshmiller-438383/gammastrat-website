...
const SHOPIFY_VARIANTS: Record<string, { variantId: string; planId: string }> = {
  ...
  Starter: { variantId: '48103900414193', planId: 'free-trial' },
}

const fallbackPlans: Plan[] = [
  { _id: 'plan-free', name: 'Free Trial', price: '$0', period: '/mth', billing: 'Free forever.', highlight: false, ctaText: 'Get started free', order: 1, features: ['Basic options scanner', '5 watchlist symbols', 'Daily market summary', 'Community access', 'Mobile app access'] },
  { _id: 'plan-basic', name: 'Basic', price: '$49', period: '/mth', billing: 'Billed monthly.', highlight: false, ctaText: 'Start free trial', order: 2, features: ['Everything in Free Trial', 'Real-time gamma exposure data', 'Unlimited watchlist symbols', 'AI-powered trade alerts', 'Options flow scanner', 'Dark pool activity feed', 'Priority email support'] },
  { _id: 'plan-gamma', name: 'Gamma', price: '$175', period: '/mth', billing: 'Billed monthly.', highlight: true, ctaText: 'Subscribe now', order: 3, features: ['Everything in Basic', 'Institutional-grade data feeds', 'Custom alert configurations', 'API access (10k calls/day)', '1-on-1 onboarding session', 'Dedicated account manager', 'White-glove support 24/7'] },
]
      ...
      <section className="pb-24 bg-[#0D0F12]">
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl">
            {plans.map((plan) => {
              const shopify = SHOPIFY_VARIANTS[plan.name] || {
                variantId: plan.shopifyVariantId,
                planId: plan.shopifyPlanId || plan.name.toLowerCase().replace(/\s+/g, '-'),
              }
              return (
